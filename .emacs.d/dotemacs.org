...
**** Keybindings
     ...
     *IMPORTANT! POTENTIAL BUG:*
     ...
     #+BEGIN_SRC emacs-lisp
       (setq ehc/emacs-lisp-mode-keybindings
             '(("s-i" . eval-last-sexp)))

       (add-hook 'emacs-lisp-mode-hook
                 '(lambda ()
                    (dolist
                        (bindings ehc/emacs-lisp-mode-keybindings)
                      (local-set-key
                       (kbd (car bindings)) (cdr bindings)))))
     #+END_SRC
...
*** Pairs
    ...
    #+BEGIN_SRC emacs-lisp
      (setq ehc/electric-pair-modes '(c++-mode
                                      emacs-lisp-mode
                                      java-mode
                                      lisp-interaction-mode
                                      sh-mode))

      (defun ehc/electric-pair-disabled-modes (mode)
        (not (member major-mode ehc/electric-pair-modes)))

      (setq electric-pair-inhibit-predicate
            'ehc/electric-pair-disabled-modes)
